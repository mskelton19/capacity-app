            ...
            {PHASE_LABELS.map((phase, i) => {
              ...
              return (
                <div key={i} style={{ position: "absolute", left: `${left}%`, width: `${width}%`, top: 0, bottom: 0, background: phase.bg, borderRight: i < PHASE_LABELS.length - 1 ? `2px solid ${phase.border}` : "none", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minWidth: 0, padding: "0 6px", boxSizing: "border-box" }}>
                  {phase.label && <div style={{ fontSize: isMobile ? 9 : 10, fontWeight: 800, color: phase.text, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", maxWidth: "100%" }}>{phase.label}</div>}
                  {phase.sublabel && <div style={{ fontSize: isMobile ? 8 : 9, color: phase.text, opacity: 0.75, overflow: "hidden", textOverflow: "ellipsis", maxWidth: "100%", textAlign: "center", lineHeight: 1.2 }}>{phase.sublabel}</div>}
                </div>
              );
            })}
          </div>
          ...
          {rows.map((row, ri) => (
            <div key={ri} ref={(el) => { rowRefs.current[ri] = el; }} style={{ display: "flex", marginBottom: ri < rows.length - 1 ? rowGap : (isMobile ? 6 : 8) }}>
              ...
              <div ref={ri === 0 ? timelineStripRef : null} style={{ flex: 1, position: "relative", height: barHeight }}>
                {MONTHS.map((_, i) => i > 0 && (
                  <div key={i} style={{ position: "absolute", left: `${(i / MONTHS.length) * 100}%`, top: 0, bottom: 0, borderLeft: "1px dashed #e2e8f0", pointerEvents: "none", zIndex: 0 }} />
                ))}
                {row.map(track => {
                ...
                <div style={{ display: "flex", alignItems: "center" }}>
                  ...
                  <div style={{ flex: 1, display: "flex", gap: 2 }}>
                    {MONTHS.map((_, mi) => {
                      ...
                      return (
                        <button key={mi} type="button" disabled={!isEditor} onClick={() => isEditor && updateCommitment("yardai", person.name, mi, nextCommitmentValue(state))}
